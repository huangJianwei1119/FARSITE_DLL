    ...
    if ( month < 13 )
 	...
 	if ( month != 13)	{					// if hit end of windspeed data
	  ...
	  cloud = a_CI->GetWindCloud (StationNumber, count);
	    cloud =cloud>(long)99?(long)99:cloud;
   CloudCount=count;
...
HANDLE hFmsEvent;
    ...
    if ( ThreadOrder < 0 ){
      ThreadOrder = ID;
      hFmsEvent =(HANDLE) a_CI->GetFarsiteEvent(EVENT_MOIST_THREAD, ThreadOrder);
      SYSTEM_INFO sysinf;
...
// *********************************************************************
unsigned int __stdcall FmsThread::RunFmsThread(void *fmsthread)
{
